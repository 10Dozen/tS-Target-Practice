...

{
				...
				if !(isNil {_x select 4}) then {
					...
					if !(isNil {_x select 5}) then { 
						...
						if !(isNil {_x select 6}) then { 
							_tileParams = _x select 6;
							if !(isNil {_x select 7}) then { _activeTileParams = _x select 7 };
						};
					};
				};
			};
	...
	private _lineProperties = [_line, _type, _data, _textParams, _tileParams, _activeTileParams, _expression, _widthMultiplier, _args];
